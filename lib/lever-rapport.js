...
async function bouwRapportPdf(meta) {
  ...
  let rennerstype = null, rennerstypeUitleg = '';
  if (piek1 && ftp) {
    const explosiviteit = piek1 / ftp;
    const wkgVoorType = wGew ? ftp / wGew : null;
    if (explosiviteit >= 2.4) { rennerstype = 'Sprinter'; rennerstypeUitleg = 'Korte, explosieve pieken ver boven je drempel. Je kracht zit in de sprint.'; }
    else if (explosiviteit >= 1.9) { rennerstype = 'Puncheur'; rennerstypeUitleg = 'Sterk in korte, harde inspanningen boven je drempel. Ideaal voor aanvallen en korte hellingen.'; }
    else if (wkgVoorType && wkgVoorType >= 3.4) { rennerstype = 'Klimmer'; rennerstypeUitleg = 'Relatief weinig sprintkracht, maar veel vermogen per kilo. Je motor houdt het lang volhoudend zwaar.'; }
    else if (explosiviteit <= 1.5) { rennerstype = 'Tijdrijder'; rennerstypeUitleg = 'Een vlakke curve zonder grote pieken. Jij bent het sterkst op een constant, hoog tempo.'; }
    else { rennerstype = 'Allrounder'; rennerstypeUitleg = 'Geen uitgesproken piek of dal in je curve. Je kunt op meerdere manieren meedoen.'; }
  }
