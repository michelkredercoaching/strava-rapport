...
function klantHtml(naam, token, deadline, advies, isHartslag, m, vorige) {
  ...
  // Rennerstype-teaser: alleen voor vermogen-kopers, en alleen als de decoupling
  // geen sterk signaal geeft (die twee gaan al voor, zie hierboven). Allrounder
  // slaan we over, dat type heeft geen scherpe "dit zie je niet op Strava"-hook.
  // Zelfde tekst-familie als de reels ("sterk op 5 min, zwak op 20 min"), nu ook
  // in de mail i.p.v. alleen in de PDF (Michel, 21-09-2026).
  const rennerstypeVoorMail = !isHartslag
  ...
  const teaserRegel = decoupHoog
    ...
    : (rennerstypeVoorMail && EMAIL_TEASER[rennerstypeVoorMail.type])
    ? EMAIL_TEASER[rennerstypeVoorMail.type]
