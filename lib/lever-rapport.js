...
async function bouwRapportPdf(meta) {
  ...
  const rennerstypeActieTekst = rennerstype ? RENNERSTYPE_ACTIE[rennerstype] : null;
  ...
  if (decoupling !== null && !Number.isNaN(decoupling)) {
    ...
    const nietGemetenUitleg = 'We vonden geen kwalificerende duurrit (minimaal 2 uur, rustig en gelijkmatig) in je laatste 90 dagen om dit op te berekenen. Rijd zo’n rit en we kunnen dit bij je volgende analyse wel meten.';
  ...
  // moet focussen (zie RENNERSTYPE_ACTIE hierboven). Los van de decoupling-tak
  // hierboven, dus geen kruising van 3x3 varianten nodig.
