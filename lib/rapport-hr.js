...
export async function bouwRapportPdfHr(meta) {
  ...
  const zoneCardH=34+ZONE_NAAM.length*21+12+28;   // +28 voor de streefregels
  ensure(zoneCardH);
  ...
  // die stonden nergens in het rapport. Zonder streefgetal weet de sporter niet
  ...
  const streefKleur=(goed)=>goed?GROEN:ORANJE;
  const sy=y-38-(ZONE_NAAM.length-1)*21-20;
  txt('STREEF',M+16,sy,{font:bold,size:7.5,color:DIM});
  ...
  txt(`rustig ${laagTot}%`,M+96,sy-13,{font:bold,size:7.5,color:streefKleur(laagTot>=75)});
  txt(`grijs ${grijsTot}%`,M+226,sy-13,{font:bold,size:7.5,color:streefKleur(grijsTot<=12)});
  txt(`kwaliteit ${Math.round(kwalAandeel)}%`,M+356,sy-13,{font:bold,size:7.5,color:streefKleur(kwalAandeel>=13&&kwalAandeel<=30)});
  y-=zoneCardH+14;
