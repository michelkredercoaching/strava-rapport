...
export async function bouwRapportPdfHr(meta) {
  ...
  // Betrouwbaarheid: 'hoog' uit hartslag-streams, anders 'schatting'.
  const opBron = meta.omslagpuntBetrouwbaarheid;
  let betr = op ? 'hoog' : '—', betrK = GROEN;
  if (op && opBron !== 'hoog') { betr='schatting'; betrK=ORANJE; }
  ...
  const pillW=138; page.drawRectangle({x:R-16-pillW,y:y-72,width:pillW,height:20,color:ORANJE});
  txt('GEEN INSPANNINGSTEST NODIG',R-16-pillW/2,y-66,{font:bold,size:7.5,color:WIT,align:'center'});
  y-=opH+14;
