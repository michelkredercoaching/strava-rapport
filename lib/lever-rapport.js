...
async function bouwRapportPdf(meta) {
  ...
  // Bij bron==='gemiddeld' is de reden altijd dezelfde check (api/strava-callback.js
  // kiesPieken): geen HR-geverifieerde 12-/20-min inspanning gevonden. Hebben we
  // die HR-cijfers en het omslagpunt bij de hand, dan citeren we ze letterlijk
  // i.p.v. de vage "je data mist een volle inspanning" — concreet bewijs overtuigt
  // beter dan een aanname (Joris, 22-09-2026: 157/153 bpm tegen een drempel van
  // 168 bpm). De andere routes naar niet-'hoog' (te weinig ritten, of géén HR-
  // data) hebben geen HR-gat om te citeren, dus die vallen terug op de generieke
  // tekst.
  const piek12Hr = parseInt(meta.piek12minHr) || null;
  const piek20Hr = parseInt(meta.piek20minHr) || null;
  const opHfVoorOndergrens = parseInt(meta.omslagpunt) || null;
  let ondergrensTekst = 'Dit is een ondergrens. Je data mist een volle inspanning, dus je echte FTP ligt waarschijnlijk hoger. Rijd één keer voluit (20 min) en je weet het zeker.';
  if (bron === 'gemiddeld' && opHfVoorOndergrens && (piek12Hr || piek20Hr)) {
    const stukken = [];
    if (piek12Hr) stukken.push(`12 min op ${piek12Hr} bpm`);
    if (piek20Hr) stukken.push(`20 min op ${piek20Hr} bpm`);
    ondergrensTekst = `Dit is een ondergrens. Je zwaarste inspanning van ${stukken.join(' en ')} bleef onder je drempel van ${opHfVoorOndergrens} bpm — geen bewijs van een echte maximale inspanning, dus je echte FTP ligt waarschijnlijk hoger. Rijd één keer voluit (20 min) en je weet het zeker.`;
  }
  const ondergrensRegels = (ftp && !ftpIsHoog)
    ? wrapTxt(ondergrensTekst, reg, 8.5, R-M-32)
    : [];
  ...
  if (meta.vorige && meta.vorige.ftp && ftp) {
    ...
    if (vorigeFtp) {
      ...
      const decRegel = (vorigeDecoupling != null && decoupling != null)
        ...
        : null;
      // Vervolgregel: nodigt uit om over 90 dagen (dezelfde vensterbreedte als
      // de analyse zelf) opnieuw te meten, ongeacht of de klant vooruit- of
      // achteruitging. Stond hier nog niet — de kaart liet zien WAT er
      // veranderde, maar zei niet wat de klant nu moest doen (Leon, 23-09-2026).
      const vervolgLines = wrapTxt('Rijd de komende 90 dagen door en doe dan opnieuw een analyse om je vooruitgang te checken.', reg, 8.5, R-M-32);
