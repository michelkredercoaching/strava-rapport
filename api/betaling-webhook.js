...
//
// Bezorging + retry (punt 1) en ontdubbeling (punt 2):
//   - Kwam de klantmail niet aan? → 503, zodat Mollie het later opnieuw aanbiedt.
//   - Eenmaal succesvol verzonden? → vastgelegd in Redis, dus een latere
//     Mollie-retry stuurt NOOIT een tweede mail.
//   - Twee webhooks tegelijk voor dezelfde betaling? → een korte lock zorgt dat
//     er maar één tegelijk verwerkt; de ander komt later (of ziet 'klaar').
//   - Geen Redis geconfigureerd of Redis plat? → alles blijft werken, alleen
//     zonder dubbel-bescherming (bewuste keuze: nooit een verkoop blokkeren).
//
...
// Werkt via een simpele HTTP-call, geen extra package nodig. Faalt 'ie of staat
// 'ie niet aan, dan degraderen alle helpers netjes naar "gewoon doorgaan".
...
// Al eerder volledig verstuurd? (overleeft Mollie-retries, 30 dagen bewaard)
...
// Lock pakken. 'vrij' = wij mogen verwerken, 'bezig' = ander handelt het af.
// Bij Redis uit/fout: 'vrij' (niet blokkeren).
...
// Beperk PDF-fail-waarschuwingen tot max 1 per uur per betaling (anti-spam).
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(503).send('mollie onbereikbaar'); // → Mollie probeert later opnieuw
  }
  ...
  try {
    ...
    // 6) Klant-adres aanwezig én mail mislukt → 503 zodat Mollie retryt (nog NIET
