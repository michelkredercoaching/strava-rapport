...
// Logo tijdelijk leeg: de vorige base64 was corrupt en liet pdf-lib oneindig hangen
// (oorzaak van de FUNCTION_INVOCATION_TIMEOUT). Zonder logo valt de PDF netjes
...
function isGeldigePng(buf) {
  try {
    ...
    while (i + 8 <= buf.length) {
      ...
      if (!/^[A-Za-z]{4}$/.test(type)) return false;       // rommel-chunk = corrupt
      if (i + 12 + len > buf.length) return false;          // chunk valt buiten bestand
      i += 12 + len;
...
async function bouwRapportPdf(meta) {
  ...
  // HEADER — logo alleen embedden als de PNG structureel klopt (anders tekst-fallback)
  ...
  if (LOGO_B64 && LOGO_B64.length>20) {
    try {
      ...
      if (isGeldigePng(logoBuf)) {
        ...
        console.error('Logo overgeslagen: base64 is geen geldige PNG');
      }
