...
async function bouwRapportPdf(meta) {
  ...
  // ===== Betrouwbaarheid + test-badge =====
  // De badge is NIET meer hardcoded. Hij volgt de betrouwbaarheid: alleen bij een
  // uit stream-data berekende, plausibele FTP staat er "GEEN FTP-TEST NODIG".
  // Bij een schatting/gemiddeld/laag staat er "DOE EERST EEN FTP-TEST" — precies
  // wat Peter-Jan had moeten zien i.p.v. "hoog / geen test nodig".
  let betr='hoog', betrK=GROEN, testNodig=false;
  const bron = meta.ftpBetrouwbaarheid;
  if (bron === 'laag') {
    betr='laag'; betrK=ROOD; testNodig=true;
  } else if (bron === 'gemiddeld') {
    betr='gemiddeld'; betrK=ORANJE; testNodig=true;
  } else if (bron === 'hoog') {
    if (ritten!=null && ritten<8) { betr='gemiddeld'; betrK=ORANJE; testNodig=true; }
    else { betr='hoog'; betrK=GROEN; }
  } else if (ritten!=null) {
    if (ritten>=15){betr='hoog';betrK=GROEN;}
    else if (ritten>=8){betr='gemiddeld';betrK=ORANJE;testNodig=true;}
    else {betr='laag';betrK=ROOD;testNodig=true;}
  }
  ...
  const badgeTxt = testNodig ? 'DOE EERST EEN FTP-TEST' : 'GEEN FTP-TEST NODIG';
  const badgeKl  = testNodig ? ROOD : ORANJE;
  const pillW = Math.max(128, bold.widthOfTextAtSize(badgeTxt,8) + 20);
  page.drawRectangle({x:R-16-pillW,y:y-72,width:pillW,height:20,color:badgeKl});
  txt(badgeTxt,R-16-pillW/2,y-66,{font:bold,size:8,color:WIT,align:'center'});
  y-=ftpH+14;
