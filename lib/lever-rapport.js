...
async function bouwRapportPdf(meta) {
  ...
  if (ftp && wGew) {
    ...
    // Rennerstype uit de curve-vorm: hoe pieky (1min t.o.v. FTP) + klimvermogen (W/kg).
    // Puur afgeleid uit data die er al is, geen nieuwe metingen nodig.
    let rennerstype = null, rennerstypeUitleg = '', rennerstypeLines = [];
    if (piek1 && ftp) {
      ...
      else if (wkg >= 3.4) { rennerstype = 'Klimmer'; rennerstypeUitleg = 'Relatief weinig sprintkracht, maar veel vermogen per kilo. Je motor houdt het lang volhoudend zwaar.'; }
      ...
      rennerstypeLines = wrapTxt(rennerstypeUitleg, reg, 8.5, R-M-32);
    }
    const ladderRijH = 15;
