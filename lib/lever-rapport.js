...
function bepaalKoperDiagnose(m, isHartslag) {
  ...
  const rennerstype = !isHartslag
    ? bepaalRennerstype(parseInt(mm.ftp)||null, parseInt(mm.piek1min)||null, (() => { const w=parseFloat(mm.weight); return (w>=35&&w<=200)?w:null; })())
    : null;
  const EMAIL_TEASER = {
...
async function bouwRapportPdf(meta) {
  ...
  const rennerstypeResultaat = bepaalRennerstype(ftp, piek1, wGew);
  ...
  const ondergrensRegels = (ftp && !ftpIsHoog)
    ? wrapTxt(alleenKortePieken
        ? 'Dit is een schatting uit je korte pieken. Je data mist een volle lange inspanning, dus dit getal kan beide kanten op: ga je kort explosief hard, dan valt het hoger uit dan je echte drempel. Rijd één keer voluit (20 min) en je weet het zeker.'
  ...
  if (ftp && wGew) {
    ...
    const ladderRijH = 15;
    const wkgH = 192 + volgLines.length*11 + (curve.length ? (13 + curve.length*13) : 0) + (rennerstype ? (14 + rennerstypeLines.length*11) : 0) + 14;
    ensure(wkgH);
...
async function naarMailchimp(m, token, deadlineNL, advies, isRetry) {
  ...
  const rennerstypeVoorMc = bepaalRennerstype(parseInt(m.ftp)||null, parseInt(m.piek1min)||null, wGewVoorMc);
