...
export function bepaalRennerstype(ftp, piek1, wGew, langePiek) {
  if (!piek1 || !ftp) return null;
  // langePiek = de 12- of 20-min piek (W). Ontbreken die allebei, dan is de FTP
  // zelf uit piek1 en piek5 afgeleid en meet piek1/ftp dus vooral de FTP-formule
  // in plaats van de renner. Erger: een te hoge FTP drukt de ratio omlaag en
  // levert 'Tijdrijder' op bij precies de explosieve renner die de schatting
  // omhoog trok. Erwin (piek1 384, FTP-schatting 272) kreeg zo "een vlakke curve
  // zonder grote pieken" te lezen, terwijl zijn test 240W gaf en hij op 1,60 zit
  // (23-09-2026). Liever geen type dan het verkeerde: dit label reist door naar
  // de mailteaser en naar Mailchimp (RENTYPE).
  if (!langePiek) return null;
...
export function bepaalCurveBadges(piek5, piek20) {
  ...
  if (piek5 && piek20) {
    ...
    // 'MATIG' i.p.v. 'ZWAK': dit is een relatief signaal (5 min t.o.v. 20 min),
    // geen absoluut oordeel — 'zwak' oogde harder dan de bedoeling (Leon,
    // 3.5 W/kg op 20 min, dat is geen zwakke renner, 23-09-2026).
    if (ratio520 >= 1.15) { badges['5 min'] = 'STERK'; badges['20 min'] = 'MATIG'; }
