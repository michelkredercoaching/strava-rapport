...
// Titels die soms voor een bankrekeningnaam staan. Die willen we niet op de
// factuur: "Hr RJ van Tjonger" moet "van Tjonger" worden, geen "Hr".
const TITELS = new Set(['hr', 'dhr', 'mr', 'mw', 'mevr', 'mevrouw', 'heer', 'de', 'den']);
// Tussenvoegsels horen bij de achternaam: "van den Hout", niet "Hout".
const TUSSEN = new Set(['van', 'de', 'der', 'den', 'ten', 'ter', 'te', 'het', "'t", 'op', 'aan', 'in', 'uit', 'du', 'le', 'la']);

// Plukt de achternaam uit een tenaamstelling van een bankrekening. Los van de
// API-aanroep hieronder, zodat de ontleding zelf te testen is.
// "H C KOREVAAR" -> "KOREVAAR", "Hr RJ van Tjonger" -> "van Tjonger".
export function achternaamUitTenaamstelling(ruw) {
  let delen = String(ruw || '').trim().split(/\s+/).filter(Boolean);
  while (delen.length > 1 && TITELS.has(delen[0].toLowerCase().replace('.', ''))) delen.shift();
  if (delen.length < 2) return delen[0] || '';
  // Vanaf het eerste tussenvoegsel is alles achternaam; anders het laatste woord.
  const i = delen.findIndex((d, n) => n > 0 && TUSSEN.has(d.toLowerCase()));
  return (i > 0 ? delen.slice(i) : [delen[delen.length - 1]]).join(' ');
}

// Haalt de achternaam uit de naam van de betaler zoals Mollie die doorgeeft
// (iDEAL levert de tenaamstelling van de rekening, andere methodes vaak niets).
// Alleen als noodgreep gebruikt: Strava's lastname is de hoofdbron.
async function betalersAchternaam(key, betaalId) {
  if (!key || !betaalId) return '';
  try {
    const r = await fetch(`https://api.mollie.com/v2/payments/${betaalId}`, {
      headers: { Authorization: `Bearer ${key}` },
      signal: AbortSignal.timeout(8000)
    });
    if (!r.ok) return '';
    const p = await r.json();
    const ruw = (p.details && (p.details.consumerName || p.details.cardHolder)) || '';
    return achternaamUitTenaamstelling(ruw);
  } catch { return ''; }
}

// Splitst een volledige naam. Gaf Strava alleen een voornaam, dan blijft de
// achternaam leeg en vult de aanroeper 'm elders aan.
 ...
 *
 * @param {object} p
 * @param {string} p.naam       voornaam van de koper (uit de Mollie-metadata)
 * @param {string} [p.achternaam] achternaam van de koper (Strava lastname)
 ...
 */
export async function maakMollieFactuur({ naam, achternaam, email, bedrag, betaalId, postcode, huisnummer, land, straat, plaats }) {
  ...
  const { voornaam, achternaam: uitNaam } = splitNaam(naam);
  const adres = String(email).trim().toLowerCase();

  // Achternaam in volgorde van betrouwbaarheid: het aparte veld uit de metadata
  // (Strava's lastname), dan wat er nog uit de volledige naam te halen viel, dan
  // de tenaamstelling van de betaling bij Mollie. Lukt niets, dan de voornaam,
  // want Mollie eist een familyName en de factuur mag nooit blijven hangen.
  // Die laatste stap gaf vroeger "Niels Niels" op elke factuur; nu is het echt
  // een noodgreep in plaats van de normale gang van zaken.
  let familienaam = String(achternaam || '').trim() || uitNaam;
  if (!familienaam) familienaam = await betalersAchternaam(KEY, betaalId);
  if (!familienaam) familienaam = voornaam;
  ...
  try {
    ...
    const nummer = data.invoiceNumber || data.number || '';

    // De omschrijving van de betaling bijwerken naar het factuurnummer. De
    // boekhouder leest de Mollie-export en kan dan matchen op factuurnummer,
    // net zoals bij de webshop. Dit kan pas hier: bij het aanmaken van de
    // betaling bestaat het nummer nog niet. Mollie accepteert een PATCH ook op
    // een al betaalde transactie (getest). Mislukt het, dan laten we het lopen:
    // de factuur is het belangrijke deel en die staat er dan al.
    if (nummer && betaalId) {
      try {
        await fetch(`https://api.mollie.com/v2/payments/${betaalId}`, {
          method: 'PATCH',
          headers: { Authorization: `Bearer ${KEY}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({ description: nummer }),
          signal: AbortSignal.timeout(8000)
        });
      } catch { /* omschrijving is bijzaak, factuur staat er al */ }
    }

    return { ok: true, id: data.id, nummer };
  } catch (e) {
