...
function klantHtml(naam, token, deadline, advies, isHartslag, m, vorige) {
  ...
  const teaserRegel = decoupHoog
    ? `Kijk eerst naar je <strong>aerobe decoupling</strong>: ${decWaarde}%. Dat is nu je belangrijkste werkpunt, zie je actieplan. Eerst duurvermogen opbouwen, dan pas extra intensiteit.`
    : decoupSterk
    ? `Je <strong>aerobe decoupling</strong> staat op ${decWaarde}%, een sterke basis. Dat fundament hoef je niet meer op te bouwen, dat is een compliment waard.`
  ...
  // vóór de rest van de mail. Alleen als het spoor van toen matcht met nu.
  let trendRegel = '';
  if (!isHartslag && vorige && vorige.ftp && mm.ftp) {
    const delta = parseInt(mm.ftp) - parseInt(vorige.ftp);
    const deltaTxt = delta > 0 ? `+${delta}W` : delta < 0 ? `${delta}W` : 'gelijk gebleven';
    trendRegel = `<p style="font-size:15px;margin:0 0 14px;">Eerst het goede nieuws: je FTP ging van ${escHtml(vorige.ftp)}W naar ${escHtml(mm.ftp)}W sinds je vorige analyse (${deltaTxt}).</p>`;
  } else if (isHartslag && vorige && vorige.omslagpunt && mm.omslagpunt) {
    trendRegel = `<p style="font-size:15px;margin:0 0 14px;">Sinds je vorige analyse ging je omslagpunt van ${escHtml(vorige.omslagpunt)} naar ${escHtml(mm.omslagpunt)} bpm.</p>`;
  }
