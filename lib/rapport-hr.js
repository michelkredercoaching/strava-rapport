// /lib/rapport-hr.js
// Bouwt het HARTSLAG-rapport (PDF) voor renners zonder (voldoende) vermogensdata.
// Spiegel van de vermogens-PDF in betaling-webhook.js, maar dan op omslagpunt
// (drempelhartslag) i.p.v. FTP: bpm-zones, RPE-gestuurde intervallen, geen W/kg.
// De webhook kiest op basis van meta.meetmethode welke van de twee hij bouwt.

export async function bouwRapportPdfHr(meta) {
  const { PDFDocument, StandardFonts, rgb } = await import('pdf-lib');
  const c = (hex) => { const n=parseInt(hex.replace('#',''),16); return rgb(((n>>16)&255)/255,((n>>8)&255)/255,(n&255)/255); };
  const BG=c('#0d0d0d'), CARD=c('#161616'), CARD2=c('#1a120c'), BORDER=c('#2b2b2b'), BORDERO=c('#48280f');
  const WIT=c('#fafafa'), MUT=c('#9a9a9a'), DIM=c('#6a6a6a'), ORANJE=c('#ff6b1a'), GROEN=c('#4caf80'), ROOD=c('#e87070');
  const TRACK=c('#262626');
  // 5 hartslagzones, kleur oplopend qua intensiteit.
  const ZONE_KLEUR=['#22c55e','#3b82f6','#eab308','#ff6b1a','#8b5cf6'];
  const ZONE_NAAM=['Herstel','D1','D2','D3','Weerstand'];

  const doc = await PDFDocument.create();
  const PW=595.28, PH=841.89, M=40, R=PW-M;
  const reg = await doc.embedFont(StandardFonts.Helvetica);
  const bold = await doc.embedFont(StandardFonts.HelveticaBold);

  let page, y;
  const nieuwePagina = () => { page = doc.addPage([PW,PH]); page.drawRectangle({x:0,y:0,width:PW,height:PH,color:BG}); y = PH - M; };
  nieuwePagina();
  const ensure = (nodig) => { if (y - nodig < 46) nieuwePagina(); };

  const txt=(s,x,yy,{font=reg,size=10,color=WIT,align='left'}={})=>{ s=String(s); let xx=x; if(align==='right')xx=x-font.widthOfTextAtSize(s,size); if(align==='center')xx=x-font.widthOfTextAtSize(s,size)/2; page.drawText(s,{x:xx,y:yy,size,font,color}); };
  const card=(x,yy,w,h,{fill=CARD,border=BORDER}={})=>page.drawRectangle({x,y:yy,width:w,height:h,color:fill,borderColor:border,borderWidth:1});
  const wrapTxt=(s,font,size,maxW)=>{ const words=s.split(' '),lines=[]; let line=''; for(const w of words){ const t=line?line+' '+w:w; if(font.widthOfTextAtSize(t,size)>maxW){lines.push(line);line=w;}else line=t;} if(line)lines.push(line); return lines; };
  const veiligPdfTekst=(s)=>String(s==null?'':s).replace(/[^\x20-\x7E\xA0-\xFF]/g,'').trim()||'Sporter';
  // Zet een 'YYYY-MM-DD'-datum (uit Strava's start_date_local, zie strava-callback.js
  // datumVanRit) om naar "12 augustus". Ankert op het middaguur in UTC zodat een
  // tijdzoneconversie de datum niet per ongeluk een dag verschuift.
  const datumKort=(yyyymmdd)=>{ if(typeof yyyymmdd!=='string'||yyyymmdd.length<10)return null; const d=new Date(yyyymmdd.slice(0,10)+'T12:00:00Z'); return Number.isNaN(d.getTime())?null:d.toLocaleDateString('nl-NL',{day:'numeric',month:'long',timeZone:'UTC'}); };

  const naam=veiligPdfTekst(meta.naam||'Sporter');
  const op=parseInt(meta.omslagpunt)||null;           // omslagpunt in bpm
  const maxHf=parseInt(meta.maxHf)||null;
  const score=meta.score!=null&&meta.score!==''?meta.score:null;
  const uren=meta.uren!=null&&meta.uren!==''?Number(meta.uren):null;
  const vo2=meta.vo2max!=null&&meta.vo2max!==''?Number(meta.vo2max):null;
  const ritten=meta.ritten!=null&&meta.ritten!==''?Number(meta.ritten):null;
  // Op het HR-spoor levert strava-callback 5 zones (index 5 blijft 0). We pakken
  // de eerste vijf; ontbreekt er data, dan tonen we de kaart met nullen.
  const zones=(meta.zones||'').split('-').map(n=>parseInt(n)||0).slice(0,5);
  while (zones.length<5) zones.push(0);

  // ---------- HEADER ----------
  txt('MICHEL KREDER COACHING',M,y-10,{font:bold,size:12});
  const datum=new Date().toLocaleDateString('nl-NL',{day:'numeric',month:'long',year:'numeric'});
  txt('TRAININGSRAPPORT',R,y-2,{font:bold,size:8,color:DIM,align:'right'});
  txt(`Analyse · laatste 90 dagen${ritten?` · ${ritten} ritten`:''}`,R,y-14,{font:reg,size:8,color:DIM,align:'right'});
  txt(datum,R,y-26,{font:reg,size:8,color:DIM,align:'right'});
  y-=46;
  txt('Hartslagprofiel',M,y-26,{font:bold,size:30});
  txt('Persoonlijk hartslagprofiel voor '+naam,M,y-42,{font:reg,size:10,color:MUT});
  if (score!=null){ txt(String(score),R,y-30,{font:bold,size:46,color:ORANJE,align:'right'}); txt('TRAINING SCORE',R,y-44,{font:bold,size:7,color:DIM,align:'right'}); }
  y-=58;
  txt('INZICHT.',M,y,{font:bold,size:10,color:WIT});
  txt(' TRAIN GERICHT.',M+bold.widthOfTextAtSize('INZICHT.',10),y,{font:bold,size:10,color:ORANJE});
  txt(' WORD STERKER.',M+bold.widthOfTextAtSize('INZICHT. TRAIN GERICHT.',10),y,{font:bold,size:10,color:WIT});
  y-=16; page.drawLine({start:{x:M,y},end:{x:R,y},thickness:1,color:BORDER}); y-=18;

  // ---------- OMSLAGPUNT DETECTOR ----------
  const opH=92; ensure(opH);
  card(M,y-opH,R-M,opH,{fill:CARD2,border:BORDERO});
  txt('OMSLAGPUNT DETECTOR',M+16,y-20,{font:bold,size:9,color:ORANJE});
  txt(op?String(op):'—',M+16,y-58,{font:bold,size:40,color:WIT});
  if(op)txt('BPM',M+16+bold.widthOfTextAtSize(String(op),40)+8,y-58,{font:bold,size:13,color:DIM});
  // Betrouwbaarheid: 'hoog' uit hartslag-streams, anders 'schatting'. De badge
  // is NIET meer hardcoded: bij een schatting (of geen omslagpunt) staat er
  // "DOE EERST EEN VELDTEST" i.p.v. "GEEN INSPANNINGSTEST NODIG".
  const opBron = meta.omslagpuntBetrouwbaarheid;
  let betr = op ? 'hoog' : '—', betrK = GROEN, testNodig = false;
  if (op && opBron !== 'hoog') { betr='schatting'; betrK=ORANJE; testNodig=true; }
  if (!op) testNodig = true;
  txt(`Je drempelhartslag${maxHf?` · max ${maxHf} bpm`:''}`,M+16,y-76,{font:reg,size:8.5,color:MUT});
  txt('Betrouwbaarheid: ',R-16-bold.widthOfTextAtSize(betr,8.5),y-20,{font:reg,size:8.5,color:MUT,align:'right'});
  txt(betr,R-16,y-20,{font:bold,size:8.5,color:betrK,align:'right'});
  const badgeTxt = testNodig ? 'DOE EERST EEN VELDTEST' : 'GEEN INSPANNINGSTEST NODIG';
  const badgeKl  = testNodig ? ROOD : ORANJE;
  const pillW = Math.max(138, bold.widthOfTextAtSize(badgeTxt,7.5) + 20);
  page.drawRectangle({x:R-16-pillW,y:y-72,width:pillW,height:20,color:badgeKl});
  txt(badgeTxt,R-16-pillW/2,y-66,{font:bold,size:7.5,color:WIT,align:'center'});
  y-=opH+14;

  // ---------- HARTSLAGZONES (verdeling + bpm-grenzen) ----------
  const zoneCardH=34+ZONE_NAAM.length*21+12+28;   // +28 voor de doelregels
  ensure(zoneCardH);
  card(M,y-zoneCardH,R-M,zoneCardH);
  txt('JOUW HARTSLAGZONES · WAAR TRAINDE JIJ? (bpm)',M+16,y-20,{font:bold,size:9,color:WIT});
  const z=(i)=>zones[i]||0;
  const laagTot=z(0)+z(1), grijsTot=z(2), kwalTot=z(3)+z(4);
  // Zelfde fix als het vermogens-rapport: kwalAandeel is gewoon kwalTot, de
  // zones tellen al op tot 100% (Joris, 22-09-2026, zie lever-rapport.js).
  const kwalAandeel=kwalTot;
  const badgeVoor=(nm)=>{
    if(nm==='D3'||nm==='Weerstand'){ if(kwalAandeel>=13&&kwalAandeel<=30)return 'goed'; return kwalAandeel<13?'laag':'hoog'; }
    if(nm==='Herstel'||nm==='D1'){ if(laagTot>=75)return 'goed'; if(laagTot>=58)return 'ok'; return 'laag'; }
    if(nm==='D2')return grijsTot>12?'hoog':'ok';
    return 'ok';
  };
  const badgeTekst=(b)=>b==='goed'?'GOED':b==='laag'?'LAAG':b==='hoog'?'TE HOOG':'OK';
  const badgeKleur=(b)=>b==='goed'?GROEN:(b==='laag'||b==='hoog')?ORANJE:DIM;
  // bpm-grenzen t.o.v. het omslagpunt: <75 / 75-85 / 85-90 / 90-100 / >100%.
  const grenzen=(i)=>{ if(!op)return ''; if(i===0)return `< ${Math.round(op*0.75)}`; if(i===ZONE_NAAM.length-1)return `> ${op}`; const p=[[0,0.75],[0.75,0.85],[0.85,0.90],[0.90,1.00]]; const [lo,hi]=p[i]; return `${Math.round(op*lo)}–${Math.round(op*hi)}`; };
  const maxPct=Math.max(...zones,1), barX=M+150, barW=R-barX-110;
  ZONE_NAAM.forEach((nm,i)=>{ const cy=y-38-i*21; txt(nm,M+16,cy,{font:bold,size:9.5,color:WIT}); txt(grenzen(i),M+86,cy,{font:reg,size:7.5,color:DIM}); page.drawRectangle({x:barX,y:cy-2,width:barW,height:7,color:TRACK}); const fw=Math.max(barW*(z(i)/Math.max(maxPct,1)),z(i)>0?3:0); if(fw>0)page.drawRectangle({x:barX,y:cy-2,width:fw,height:7,color:c(ZONE_KLEUR[i]||'#6b7280')}); txt(`${z(i)}%`,barX+barW+26,cy,{font:bold,size:9,color:MUT,align:'right'}); const b=badgeVoor(nm); const bx=R-58; page.drawRectangle({x:bx,y:cy-3,width:54,height:13,color:CARD,borderColor:badgeKleur(b),borderWidth:0.8}); txt(badgeTekst(b),bx+27,cy,{font:bold,size:7,color:badgeKleur(b),align:'center'}); });
  // ===== DOELPERCENTAGES =====
  // De badges hierboven (LAAG / TE HOOG / GOED) rekenen met vaste drempels, maar
  // die stonden nergens in het rapport. Zonder doelgetal weet de sporter niet
  // waar hij naartoe moet (Niels, 2 september 2026). Nu staan ze eronder, met zijn
  // eigen verdeling ernaast: groen als het klopt, oranje als het niet klopt.
  // De getallen komen uit exact dezelfde variabelen als de badges, zodat de regel
  // en de badge elkaar nooit kunnen tegenspreken.
  const doelKleur=(goed)=>goed?GROEN:ORANJE;
  const sy=y-38-(ZONE_NAAM.length-1)*21-20;
  txt('DOEL',M+16,sy,{font:bold,size:7.5,color:DIM});
  txt('rustig 75% of meer',M+96,sy,{font:reg,size:7.5,color:DIM});
  txt('grijs 12% of minder',M+226,sy,{font:reg,size:7.5,color:DIM});
  txt('kwaliteit 13 tot 30%',M+356,sy,{font:reg,size:7.5,color:DIM});
  txt('JIJ',M+16,sy-13,{font:bold,size:7.5,color:WIT});
  txt(`rustig ${laagTot}%`,M+96,sy-13,{font:bold,size:7.5,color:doelKleur(laagTot>=75)});
  txt(`grijs ${grijsTot}%`,M+226,sy-13,{font:bold,size:7.5,color:doelKleur(grijsTot<=12)});
  txt(`kwaliteit ${Math.round(kwalAandeel)}%`,M+356,sy-13,{font:bold,size:7.5,color:doelKleur(kwalAandeel>=13&&kwalAandeel<=30)});
  y-=zoneCardH+14;

  // ---------- SNELHEID:HR-DECOUPLING (Pa:HR-drift) ----------
  // Blijft op dezelfde pagina als de hartslagzones hierboven i.p.v. een eigen
  // pagina: los was dit blok te leeg op zijn eigen pagina (Michel, 21-09-2026).
  // Tegenhanger van de "AEROBE DECOUPLING"-kaart in het vermogensrapport
  // (zie lib/lever-rapport.js), maar op snelheid i.p.v. vermogen: zonder
  // vermogensmeter is er niets anders om tegen je hartslag te decouplen.
  // strava-callback.js (bepaalDecouplingHr) berekent 'm alleen op een
  // overwegend VLAKKE en RUSTIGE duurrit — vermogen compenseert vanzelf voor
  // een klim of een vlaag tegenwind, snelheid niet. Daarom is dit getal
  // gevoeliger voor omstandigheden dan de vermogensversie, en tonen we dat
  // hier expliciet: het betrouwbaarheidslabel komt uit strava-callback en is
  // NOOIT 'hoog' (dat predicaat is voorbehouden aan de vermogensversie), plus
  // een korte voetnoot die dat uitlegt. Alleen aanwezig als er een
  // kwalificerende rit was — anders toont deze kaart niets (geen gok tonen).
  const decouplingHr = meta.decouplingHr !== undefined && meta.decouplingHr !== '' && meta.decouplingHr !== null ? parseFloat(meta.decouplingHr) : null;
  const decouplingHrMin = parseInt(meta.decouplingHrMin) || null;
  const decouplingHrDatumStr = datumKort(meta.decouplingHrDatum);
  const decouplingHrBetr = meta.decouplingHrBetr === 'gemiddeld' ? 'gemiddeld' : 'laag';   // nooit 'hoog', zie hierboven
  if (decouplingHr !== null && !Number.isNaN(decouplingHr)) {
    let decLabel, decKleur, decPil;
    if (decouplingHr <= 5) { decLabel='sterk'; decKleur=GROEN; decPil='STERKE AEROBE BASIS'; }
    else if (decouplingHr <= 10) { decLabel='gemiddeld'; decKleur=ORANJE; decPil='RUIMTE VOOR GROEI'; }
    // Zelfde fix als het vermogens-rapport: dit label beschrijft de aerobe
    // basis, niet de drift (Joris, 22-09-2026, zie lever-rapport.js).
    else { decLabel='zwak'; decKleur=ROOD; decPil='BOUW EERST JE AEROBE BASIS'; }
    const decUitleg = decouplingHr <= 5
      ? 'Je hartslag bleef stabiel bij gelijkblijvende snelheid — een goed teken voor je aerobe basis op de lange afstand.'
      : decouplingHr <= 10
      ? 'Je hartslag liep iets op bij gelijkblijvende snelheid. Geen alarm, maar met meer duurkilometers onder je basistempo daalt dit verder.'
      : 'Je hartslag liep flink op bij gelijkblijvende snelheid — een teken dat je aerobe basis de duur nog niet aankan, of dat wind, terrein, hitte of te weinig vocht een rol speelde in deze rit.';
    const decFootnote = 'Gebaseerd op snelheid, niet op vermogen (je hebt geen vermogensmeter) — daardoor gevoeliger voor wind en terrein dan een vermogens-decoupling. Zie dit als een indicatie, niet als een harde meting.';
    const decLines = wrapTxt(decUitleg, reg, 8.5, R-M-32);
    const decFootLines = wrapTxt(decFootnote, reg, 7.5, R-M-32);
    const decH = 104 + decLines.length*11 + 6 + decFootLines.length*10 + 10;
    ensure(decH);
    card(M,y-decH,R-M,decH,{fill:CARD2,border:BORDERO});
    txt('SNELHEID:HR-DECOUPLING', M+16, y-20, {font:bold,size:9,color:ORANJE});
    const decStr = `${decouplingHr}%`;
    txt(decStr, M+16, y-58, {font:bold,size:40,color:WIT});
    txt('Pa:HR-DRIFT', M+16+bold.widthOfTextAtSize(decStr,40)+8, y-58, {font:bold,size:13,color:DIM});
    txt(`Over je langste vlakke duurrit${decouplingHrMin?` · ${decouplingHrMin} min`:''}${decouplingHrDatumStr?` · ${decouplingHrDatumStr}`:''}`, M+16, y-76, {font:reg,size:8.5,color:MUT});
    txt('Aerobe basis: ', R-16-bold.widthOfTextAtSize(decLabel,8.5)-reg.widthOfTextAtSize('Aerobe basis: ',8.5), y-20, {font:reg,size:8.5,color:MUT});
    txt(decLabel, R-16, y-20, {font:bold,size:8.5,color:decKleur,align:'right'});
    const decPilW = Math.max(150, bold.widthOfTextAtSize(decPil,7.5)+20);
    page.drawRectangle({x:R-16-decPilW,y:y-72,width:decPilW,height:20,color:decKleur});
    txt(decPil, R-16-decPilW/2, y-66, {font:bold,size:7.5,color:WIT,align:'center'});
    // ===== BETROUWBAARHEID — expliciet en eerlijk, i.p.v. verstopt =====
    const decBetrKleur = decouplingHrBetr==='gemiddeld' ? ORANJE : ROOD;
    txt('DOEL', M+16, y-90, {font:bold,size:7.5,color:DIM});
    txt('0–5%', M+50, y-90, {font:reg,size:7.5,color:DIM});
    txt('JIJ', M+110, y-90, {font:bold,size:7.5,color:WIT});
    txt(`${decouplingHr}%`, M+140, y-90, {font:bold,size:7.5,color:decouplingHr<=5?GROEN:ORANJE});
    txt('BETROUWBAARHEID', M+230, y-90, {font:bold,size:7.5,color:DIM});
    txt(decouplingHrBetr, R-16, y-90, {font:bold,size:7.5,color:decBetrKleur,align:'right'});
    decLines.forEach((ln,i)=> txt(ln, M+16, y-104-i*11, {font:reg,size:8.5,color:MUT}));
    const footY = y-104-decLines.length*11-8;
    decFootLines.forEach((ln,i)=> txt(ln, M+16, footY-i*10, {font:reg,size:7.5,color:DIM}));
    y -= decH + 14;
  } else {
    // Niet gemeten: EXPLICIET tonen i.p.v. de kaart weg te laten, met 0% en een
    // duidelijke reden, zodat de koper niet denkt dat dit vergeten is
    // (Michel, 21-09-2026, zelfde principe als de vermogensversie).
    // Gepersonaliseerd met de reden (api/strava-callback.js decouplingHrReden),
    // zelfde onderscheid als het vermogensrapport (Leon, 23-09-2026).
    const langsteRitMin = parseInt(meta.langsteRitMinuten) || null;
    const langsteRitTekst = langsteRitMin == null ? null : langsteRitMin < 60
      ? `${langsteRitMin} minuten`
      : `${Math.floor(langsteRitMin/60)} uur${langsteRitMin%60 ? ` en ${langsteRitMin%60} minuten` : ''}`;
    const nietGemetenUitleg = (meta.decouplingHrReden === 'te_kort' && langsteRitTekst)
      ? `Je langste rit in de laatste 90 dagen duurde ${langsteRitTekst}. We hebben minimaal 2 uur nodig om dit te meten: rijd een keer langer dan die rit en we kunnen dit bij je volgende analyse wel berekenen.`
      : meta.decouplingHrReden === 'te_onregelmatig'
      ? 'Je langste rit was wel lang genoeg, maar niet vlak en rustig genoeg (bijvoorbeeld veel heuvels of stop-and-go) om dit betrouwbaar te meten. Rijd een keer bewust vlak en rustig door en we kunnen dit bij je volgende analyse wel berekenen.'
      : 'We vonden geen kwalificerende vlakke, rustige duurrit in je laatste 90 dagen om dit op te berekenen. Rijd zo’n rit en we kunnen dit bij je volgende analyse wel meten.';
    const nietGemetenLines = wrapTxt(nietGemetenUitleg, reg, 8.5, R-M-32);
    const decH = 92 + nietGemetenLines.length*11 + 6;
    ensure(decH);
    card(M,y-decH,R-M,decH,{fill:CARD2,border:BORDERO});
    txt('SNELHEID:HR-DECOUPLING', M+16, y-20, {font:bold,size:9,color:ORANJE});
    txt('0%', M+16, y-58, {font:bold,size:40,color:DIM});
    txt('Pa:HR-DRIFT', M+16+bold.widthOfTextAtSize('0%',40)+8, y-58, {font:bold,size:13,color:DIM});
    const nietGemetenPil = 'KON NIET WORDEN GEDETECTEERD';
    const nietGemetenPilW = Math.max(150, bold.widthOfTextAtSize(nietGemetenPil,7.5)+20);
    page.drawRectangle({x:R-16-nietGemetenPilW,y:y-72,width:nietGemetenPilW,height:20,color:DIM});
    txt(nietGemetenPil, R-16-nietGemetenPilW/2, y-66, {font:bold,size:7.5,color:WIT,align:'center'});
    nietGemetenLines.forEach((ln,i)=> txt(ln, M+16, y-84-i*11, {font:reg,size:8.5,color:MUT}));
    y -= decH + 14;
  }

  // ===== TREND CARD — sinds je vorige analyse (alleen bij een herhaalkoper) =====
  // Zelfde opzet als het vermogensrapport, maar dan op omslagpunt (bpm) en
  // Pa:HR-decoupling. LET OP, in tegenstelling tot FTP is een stijgend of
  // dalend omslagpunt niet eenduidig goed of slecht (kan ook drift/vermoeidheid
  // betekenen), dus het getal krijgt bewust GEEN groen/rood, alleen de
  // decoupling-vergelijking eronder draagt een oordeel (die conventie staat al
  // in de rest van dit rapport). Staat bewust NA de decoupling-kaart hierboven,
  // want deze regel gebruikt de daar berekende 'decouplingHr'.
  if (meta.vorige && meta.vorige.omslagpunt && op) {
    const vorigeOms = parseInt(meta.vorige.omslagpunt) || null;
    const vorigeDecHr = meta.vorige.decouplingHr !== '' && meta.vorige.decouplingHr != null ? parseFloat(meta.vorige.decouplingHr) : null;
    if (vorigeOms) {
      const deltaOms = op - vorigeOms;
      const deltaStr = deltaOms > 0 ? `+${deltaOms}` : deltaOms < 0 ? `${deltaOms}` : '±0';
      const deltaKleur = WIT;
      const decRegel = (vorigeDecHr != null && decouplingHr != null)
        ? `Decoupling (Pa:HR): ${vorigeDecHr}% -> ${decouplingHr}%`
        : null;
      // Zelfde toevoeging als het vermogensrapport: uitnodigen om opnieuw te
      // meten. Staat het omslagpunt nog op een schatting (testNodig, zie de
      // OMSLAGPUNT DETECTOR hierboven), dan is een veldtest op korte termijn
      // beter advies dan 90 dagen wachten (Leon/Tjalling, 23-09-2026, zie
      // lever-rapport.js).
      const vervolgTekst = testNodig
        ? 'Je omslagpunt staat nog op een schatting. Doe binnen nu en een maand een veldtest en check je omslagpunt dan nog een keer.'
        : 'Rijd de komende 90 dagen door en doe dan opnieuw een analyse om je vooruitgang te checken.';
      const vervolgLines = wrapTxt(vervolgTekst, reg, 8.5, R-M-32);
      const laatsteVasteRegelOffset = decRegel ? 92 : 76;
      const vervolgOffset = laatsteVasteRegelOffset + 16;
      const trendH = vervolgOffset + (vervolgLines.length-1)*11 + 14;
      ensure(trendH);
      card(M,y-trendH,R-M,trendH,{fill:CARD2,border:BORDERO});
      txt('SINDS JE VORIGE ANALYSE', M+16, y-20, {font:bold,size:9,color:ORANJE});
      if (meta.vorige.datum) txt(meta.vorige.datum, R-16, y-20, {font:reg,size:8,color:DIM,align:'right'});
      txt(deltaStr, M+16, y-58, {font:bold,size:40,color:deltaKleur});
      txt('BPM OMSLAGPUNT', M+16+bold.widthOfTextAtSize(deltaStr,40)+8, y-58, {font:bold,size:11,color:DIM});
      txt(`${vorigeOms} bpm -> ${op} bpm`, M+16, y-76, {font:reg,size:8.5,color:MUT});
      if (decRegel) txt(decRegel, M+16, y-92, {font:reg,size:8.5,color:MUT});
      vervolgLines.forEach((ln,i)=> txt(ln, M+16, y-vervolgOffset-i*11, {font:reg,size:8.5,color:MUT}));
      y -= trendH + 14;
    }
  }

  // ===== PAGINA 2: AANDACHTSPUNT + ACTIEPLAN + INTERVALBLOKKEN + WEEK =====
  nieuwePagina();

  // ---------- KRITIEKE BEVINDING / AANDACHTSPUNT: (bijna) geen sessies boven omslagpunt ----------
  if (vo2!=null && Number(vo2)<=1) {
    const nulVo2 = Number(vo2)===0;
    const boodschap = nulVo2
      ? 'In 90 dagen deed je 0 sessies boven je omslagpunt. Dit is meestal de hoofdoorzaak van een prestatieplateau — je motor krijgt geen groeiprikkel.'
      : `In 90 dagen deed je maar ${vo2} sessie boven je omslagpunt. Dat mag vaker: dit is de meest tijdsefficiënte manier om je plafond te verleggen.`;
    const lines=wrapTxt(boodschap,reg,9.5,R-M-32);
    const kh=24+lines.length*13+10; ensure(kh);
    card(M,y-kh,R-M,kh,{fill: nulVo2?c('#1c1010'):c('#1c1810'), border: nulVo2?c('#5a2424'):c('#5a4a24')});
    txt(nulVo2?'KRITIEKE BEVINDING':'AANDACHTSPUNT',M+16,y-18,{font:bold,size:8,color: nulVo2?ROOD:ORANJE});
    lines.forEach((ln,idx)=>txt(ln,M+16,y-32-idx*13,{font:reg,size:9.5,color: nulVo2?c('#d8b0b0'):c('#d8c8a8')}));
    y-=kh+14;
  }

  // ---------- ACTIEPLAN (op gevoel/RPE) ----------
  // Spiegel van het vermogensrapport, maar dan in bpm. D1 loopt van 75 tot 85%
  // van je omslagpunt, daarboven begint het grijze gebied.
  const bRustig = op ? `${Math.round(op*0.75)}–${Math.round(op*0.85)} bpm` : null;
  const bGrijs  = op ? `${Math.round(op*0.85)} bpm` : null;
  const rustigRegel = bRustig?`Rijd je rustige ritten op ${bRustig} (D1). Boven ${bGrijs} zit je in het grijze gebied: te zwaar om van te herstellen, te licht om van te groeien.`:'Rijd je rustige ritten écht rustig, je kunt volledige zinnen praten. Blijf uit het grijze gebied.';
  const vo2Regel = op?`Voeg 1x per week een blok boven je omslagpunt toe: korte intervallen op RPE 9/10 (Weerstand, > ${op} bpm), bijvoorbeeld 30 sec aan en 30 sec uit.`:'Voeg 1x per week een intervalblok toe: korte intervallen op RPE 9/10, bijvoorbeeld 30 sec aan en 30 sec uit.';
  // Spoorbewust actieplan op basis van de Pa:HR-decoupling hierboven, zelfde
  // redenering als het vermogensrapport: een hoge decoupling maakt "voeg een
  // blok toe" een slecht advies, en een sterke decoupling verdient een
  // compliment in plaats van een correctie.
  const decoupHoog  = decouplingHr !== null && decouplingHr > 10;
  const decoupSterk = decouplingHr !== null && decouplingHr <= 5;
  let acties;
  if (decoupHoog) {
    acties = [
      'Duurvermogen is nu je belangrijkste werk, niet extra intensiteit. Je hartslag liep flink op tijdens je langste rit, een teken dat je aerobe basis de duur nog niet aankan.',
      rustigRegel,
      'Laat het blok boven je omslagpunt deze weken links liggen. Bouw eerst 4 tot 6 weken puur op duurvermogen, daarna pakt een intervalblok een stuk beter.'
    ];
  } else if (decoupSterk) {
    acties = [
      'Je aerobe basis staat er sterk voor, dat is een compliment waard. Dat fundament hoef je niet meer op te bouwen, dus ga hier vooral mee door.',
      rustigRegel,
      vo2Regel
    ];
  } else {
    acties = [
      'Train minimaal 3x per week — consistent, elke week, geen uitzonderingen.',
      rustigRegel,
      vo2Regel
    ];
  }
  const actLines=acties.map(a=>wrapTxt(a,reg,9.5,R-M-58));
  const actH=24+actLines.reduce((s,l)=>s+Math.max(l.length*12,12)+8,0)+6;
  ensure(actH);
  card(M,y-actH,R-M,actH);
  txt('JOUW ACTIEPLAN',M+16,y-18,{font:bold,size:9,color:WIT});
  let ay=y-36;
  actLines.forEach((lines,i)=>{ page.drawCircle({x:M+24,y:ay-3,size:9,color:ORANJE}); txt(String(i+1),M+24,ay-6,{font:bold,size:8,color:WIT,align:'center'}); lines.forEach((ln,idx)=>txt(ln,M+44,ay-idx*12,{font:reg,size:9.5,color:MUT})); ay-=Math.max(lines.length*12,12)+8; });
  y-=actH+14;

  // ---------- INTERVALBLOKKEN (RPE-gestuurd) ----------
  // Op hartslag stuur je op gevoel, niet op een exact getal: je HF loopt aan het
  // begin van een interval achter en piekt pas laat. Vandaar RPE + zone i.p.v. bpm.
  const meerVolume = (uren||0) >= 8;
  const blokken=[];
  // Spoorbewust: bij een hoge decoupling is "kies een blok boven je omslagpunt"
  // precies het advies dat het actieplan hierboven afraadt. Geen drempel of
  // VO2max deze weken, puur duurvermogen.
  if (decoupHoog) {
    blokken.push({type:'DUURVERMOGEN — VERLENGDE DUURRIT · 1X PER WEEK', oms:'Verleng je langste rit met 20 tot 30 minuten, op dezelfde rustige intensiteit', det:'Dat is precies de prikkel die je aerobe basis nu nodig heeft. Geen extra intensiteit, gewoon meer tijd in je rustige zone.'});
    blokken.push({type:'TEMPO — CONTROLEBLOK', oms:'3×15 min op D2 (net boven duurtempo) · RPE 5/6 · 5 min rust', det:'Ruim onder je omslagpunt. Dit is de enige "kwaliteit" die je deze weken nodig hebt, de rest is volume.'});
  } else if (meerVolume){
    blokken.push({type:'DUURKRACHT — DREMPELBLOK · 1X PER WEEK', oms:'5×8 min op D3 (net onder je omslagpunt) · RPE 7/8 · 4 min rust', det:'Bouwt je aerobe motor en drempel zonder je leeg te trekken. Comfortabel zwaar — je kunt nog korte zinnen praten, niet meer dan dat. Één keer per week is genoeg.'});
    blokken.push({type:'VO2MAX — OPTIE A · LANGE MICRO-INTERVALLEN', oms:'40-20 op Weerstand · RPE 9/10 · 2–3 blokken van 8–12 min, 5 min rust', det:'40 sec vol, 20 sec rustig dóórdraaien. Je hartslag kruipt over het blok richting je max — maximale prikkel voor je zuurstofopname. 1x per week.'});
    blokken.push({type:'VO2MAX — OPTIE B · IETS LANGERE REPS', oms:'60-30 op Weerstand · RPE 9 · 2–3 blokken van 8–12 min, 5 min rust', det:'60 sec stevig, 30 sec rustig dóórdraaien. Iets langer aan dan de 40-20, maar kort genoeg om scherp te blijven. Kies één optie per week.'});
  } else {
    blokken.push({type:'DUURKRACHT — DREMPELBLOK · 1X PER WEEK', oms:'5×5 min op D3 (net onder je omslagpunt) · RPE 7/8 · 3 min rust', det:'De efficiënte manier om je drempel te verhogen als je weinig tijd hebt. Comfortabel zwaar, niet vol. Één keer per week — vaker is niet nodig.'});
    blokken.push({type:'VO2MAX — OPTIE A · KORTE MICRO-INTERVALLEN', oms:'20-10 op Weerstand · RPE 9 · 2–3 blokken van 8–12 min, 5 min rust', det:'20 sec aan, 10 sec uit. Toegankelijk maar effectief — je houdt het vol terwijl de prikkel hoog blijft. Ideaal als je minder fietst. 1x per week.'});
    blokken.push({type:'VO2MAX — OPTIE B · PITTIGER', oms:'30-30 op Weerstand · RPE 9/10 · 2–3 blokken van 8–12 min, 5 min rust', det:'30 sec stevig, 30 sec rustig. Iets meer bite dan de 20-10. Kies één van beide opties per week voor afwisseling.'});
  }
  ensure(30);
  txt('INTERVALBLOKKEN VOOR JOUW NIVEAU',M,y,{font:bold,size:9,color:WIT}); y-=14;
  txt(decoupHoog ? 'Focus deze weken puur op duurvermogen. Intervallen boven je omslagpunt komen terug zodra je aerobe basis sterker staat.' : 'Kies één VO2max-variant en houd die 4 tot 6 weken vol. Daarna pas wisselen.',M,y,{font:reg,size:8,color:DIM}); y-=13;
  blokken.forEach(b=>{
    const detLines=wrapTxt(b.det,reg,8.5,R-M-32);
    const h=14+11+6+13+6+detLines.length*11+12;
    ensure(h+6);
    card(M,y-h,R-M,h);
    txt(b.type,M+16,y-18,{font:bold,size:8,color:ORANJE});
    txt(b.oms,M+16,y-34,{font:bold,size:11,color:WIT});
    detLines.forEach((ln,idx)=>txt(ln,M+16,y-50-idx*11,{font:reg,size:8.5,color:MUT}));
    y-=h+8;
  });
  y-=6;

  // ===== JOUW WEEK =====
  // Beantwoordt de vraag die het rapport zelf opriep: komt het drempelblok
  // bovenop de VO2max-sessie? (Niels, 2 september 2026.) Bij drie trainingen is
  // dat één harde dag, één stevige en één rustige, en dat staat nu zwart op wit.
  const weekRegels = decoupHoog
    ? [['Dinsdag','verlengde duurrit'],['Donderdag','tempo-controleblok'],['Zondag','lange rustige rit']]
    : meerVolume
    ? [['Dinsdag','drempelblok'],['Donderdag','VO2max, kies één variant'],['Zaterdag','lange rustige rit'],['Zondag','rustig uitrijden of vrij']]
    : [['Dinsdag','drempelblok'],['Donderdag','VO2max, kies één variant'],['Zondag','lange rustige rit']];
  const weekSlot = decoupHoog
    ? 'Geen drempel, geen intervallen boven je omslagpunt. Puur duurvermogen deze weken, dat is nu je grootste hefboom.'
    : meerVolume
    ? 'Twee stevige dagen, twee rustige. Meer intensiteit levert bij dit volume niets extra op.'
    : 'Eén harde dag, één stevige, één rustige. Meer intensiteit past niet in drie trainingen.';
  const weekSlotLines = wrapTxt(weekSlot,reg,8.5,R-M-32);
  const weekH = 26 + weekRegels.length*14 + 6 + weekSlotLines.length*11 + 12;
  ensure(weekH+6);
  card(M,y-weekH,R-M,weekH);
  txt('JOUW WEEK',M+16,y-18,{font:bold,size:9,color:WIT});
  weekRegels.forEach((r,i)=>{ const wy=y-36-i*14; txt(r[0],M+16,wy,{font:bold,size:9,color:ORANJE}); txt(r[1],M+106,wy,{font:reg,size:9,color:MUT}); });
  weekSlotLines.forEach((ln,idx)=>txt(ln,M+16,y-36-weekRegels.length*14-4-idx*11,{font:reg,size:8.5,color:DIM}));
  y-=weekH+14;

  // 'Van inzicht naar uitvoering'-CTA is bewust UIT het PDF gehaald (Michel,
  // 21-09-2026): de upsell naar het trainingsschema staat al in de klantmail
  // zelf (zie klantHtml in lever-rapport.js), dat hoeft niet dubbel in de bijlage.

  txt('Trainingsrapport · Michel Kreder Coaching · momentopname op basis van je Strava-data.',M,26,{font:reg,size:7,color:DIM});

  return await doc.save();
}
