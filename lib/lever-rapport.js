...
async function bouwRapportPdf(meta) {
  ...
  const acties=[
    ...
    wRustig?`Rijd je rustige ritten op ${wRustig}. Boven ${wGrijs} zit je in het grijze gebied: te zwaar om van te herstellen, te licht om van te groeien.`:'Rijd je rustige ritten écht rustig, je kunt volledige zinnen praten. Blijf uit het grijze gebied.',
    ftp?`Voeg 1x per week een VO2max-blok toe: korte intervallen boven ${w110}W, bijvoorbeeld 30 sec aan en 30 sec uit.`:'Voeg 1x per week een VO2max-blok toe: korte intervallen, bijvoorbeeld 30 sec aan en 30 sec uit.'
  ];
...
function interneHtml(m, bedrag, id, pdfErbij, vorige) {
  ...
  // voor Michel (staat niet in de PDF van de klant). Alleen tonen als er een
  // eerdere FTP of decoupling bekend was.
  const vorigeBalk = (vorige && (vorige.ftp || vorige.decoupling))
...
async function haalVorigeAnalyse(base, auth, hash) {
  try {
    ...
    if (!mf.FTP && !mf.DECOUPL) return null;
    return { ftp: mf.FTP || '', score: mf.SCORE || '', decoupling: mf.DECOUPL || '', datum: mf.RAPDAT || '' };
...
export async function leverRapport(m, { bedrag, id } = {}) {
  ...
  // Loopt VOOR de interne mail, want die haalt hier "vorige analyse" uit op (voor
  // een repeat-koper) en zet 'm in de interne verkoopmelding. Faalt Mailchimp,
  // dan is vorige gewoon null en verschijnt de vergelijking niet — de klant merkt
  // daar niets van, dat blokkeert alleen de eigen sales-mail van Michel.
  let vorige = null;
  try {
