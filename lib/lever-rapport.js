...
import { bepaalPijn, bepaalDecouplingSignaal, bepaalRennerstype, bepaalCurveBadges } from './pijn-signalen.js';
...
function naarHtmlEntities(s) {
  // Array.from splitst op Unicode-codepoints (niet op UTF-16 code units), dus
  // een emoji als 🚴 (een surrogate-paar) blijft heel i.p.v. dat de twee helften
  // los worden omgezet naar ongeldige entities die als "??" renderen. Zuiver
  // cosmetisch gevonden bij het testen van de nieuwe interne-mail-advies-balk
  // (Michel, 21-09-2026), maar gold al voor elke emoji in deze mails.
  return Array.from(String(s)).map(ch => {
    const cp = ch.codePointAt(0);
    return cp > 0x7F ? "&#" + cp + ";" : ch;
  }).join('');
}
// Zet een 'YYYY-MM-DD'-datum (uit Strava's start_date_local, zie strava-callback.js
// datumVanRit) om naar "12 augustus". Ankert op het middaguur in UTC zodat een
// tijdzoneconversie de datum niet per ongeluk een dag verschuift.
function datumKort(yyyymmdd) {
  if (typeof yyyymmdd !== 'string' || yyyymmdd.length < 10) return null;
  const d = new Date(yyyymmdd.slice(0, 10) + 'T12:00:00Z');
  return Number.isNaN(d.getTime()) ? null : d.toLocaleDateString('nl-NL', { day: 'numeric', month: 'long', timeZone: 'UTC' });
}

// bepaalRennerstype + bepaalCurveBadges staan nu in ./pijn-signalen.js (zie
// import hierboven), zodat api/lead.js (verlaten-journey) ze ook kan
// gebruiken zonder een circulaire import op dit bestand (Michel, 21-09-2026).

// ===== KOPER-DIAGNOSE — gedeeld tussen de klantmail (teaser + upsell-reden) en
// de interne verkoopmelding, zodat Michel precies ziet wat de klant te lezen
// kreeg i.p.v. alleen de ruwe cijfers (Michel, 21-09-2026: "ik wil dit advies
// ook zien"). Eén bron van waarheid voorkomt dat de twee mails uit elkaar lopen.
function bepaalKoperDiagnose(m, isHartslag) {
  const mm = m || {};
  ...
  const rennerstype = !isHartslag
    ? bepaalRennerstype(parseInt(mm.ftp)||null, parseInt(mm.piek1min)||null, (() => { const w=parseFloat(mm.weight); return (w>=35&&w<=200)?w:null; })(), (parseInt(mm.piek20min)||parseInt(mm.piek12min)||null))
    : null;
  const EMAIL_TEASER = {
    Sprinter: 'Je <strong>power curve</strong> laat een duidelijk profiel zien: explosief op 1 minuut, relatief zwakker op je duurvermogen. Dat is precies waar de winst zit, en dat lees je terug in je actieplan.',
    Puncheur: 'Je <strong>power curve</strong> laat zien dat je sterk bent in korte, harde pieken boven je drempel. Wat dat voor je training betekent, lees je terug in je actieplan.',
    Klimmer: 'Je <strong>power curve</strong> laat zien dat je relatief weinig sprintkracht hebt, maar veel vermogen per kilo dat je lang volhoudt. Wat dat voor je training betekent, lees je terug in je actieplan.',
    Tijdrijder: 'Je <strong>power curve</strong> is opvallend vlak: geen uitgesproken piek, een sterke motor op een constant tempo. Precies daar zit je grootste kans, en dat lees je terug in je actieplan.',
  };
  const teaserRegel = decoupHoog
    ? (isHartslag
      ...
      : `Kijk eerst naar je <strong>vermogen op je langste rit</strong>: dat zakte ${decWaarde}% weg naarmate de rit vorderde. Die ene meting vertelt je nu het meeste. Mijn advies in je actieplan is dan ook simpel: eerst je duurvermogen opbouwen, extra intensiteit komt later wel.`)
    : decoupSterk
    ? (isHartslag
      ? `Je <strong>aerobe decoupling</strong> staat op ${decWaarde}%. Sterke basis, dat mag je gewoon vasthouden.`
      : `Je <strong>vermogen bleef overeind</strong> op je langste rit, het zakte maar ${decWaarde}% weg. Sterke basis, dat mag je gewoon vasthouden.`)
    : (rennerstype && EMAIL_TEASER[rennerstype.type])
    ? EMAIL_TEASER[rennerstype.type]
  ...
  const RENNERSTYPE_UPSELL = {
    Sprinter: 'Het schema legt de nadruk op je drempel, zodat je die explosieve kracht straks langer kunt vasthouden in plaats van hem na één keer te verbruiken.',
    Puncheur: 'Het schema bouwt voort op je sterke punt, met net genoeg drempelwerk om die pieken langer vast te houden.',
    Klimmer: 'Het schema zet drempeltraining centraal, zodat elke training zich direct vertaalt naar meer watt op de klim.',
    Tijdrijder: 'Het schema geeft je top-end de aandacht die het nu mist, naast het duurvermogen waar je al sterk in bent.',
  };
  const upsellReden = decoupHoog
    ? 'Het schema bouwt eerst gericht je duurvermogen op, precies waar je nu op vastloopt, voordat het de intensiteit weer opvoert.'
    : decoupSterk
    ? 'Je aerobe basis staat al, dus het schema kan vanaf dag 1 vol inzetten op intensiteit in plaats van eerst tijd te verliezen aan opbouwen.'
    : (rennerstype && RENNERSTYPE_UPSELL[rennerstype.type])
    ? RENNERSTYPE_UPSELL[rennerstype.type]
    : 'Het schema plant precies de balans tussen rustig en pittig die je nu mist, in plaats van dat je zelf moet gokken.';
  return { decWaarde, decoupHoog, decoupSterk, rennerstype, teaserRegel, upsellReden };
}
...
async function bouwRapportPdf(meta) {
  ...
  // ===== RENNERSTYPE — uit de curve-vorm, gehoist zodat zowel de power-curve-
  // kaart (pagina 2) als het actieplan (pagina 3) 'm kunnen gebruiken. Hoe pieky
  // (1min t.o.v. FTP) + klimvermogen (W/kg). Puur afgeleid uit data die er al
  // is, geen nieuwe metingen nodig.
  ...
  const curveAlle = [['1 min',piek1],['5 min',piek5],['12 min',piek12],['20 min',piek20]];
  const curve = curveAlle.filter(pp => pp[1] && pp[1] > 0);
  // Ontbrekende vensters werden hier stil weggefilterd: Erwin zag een curve van
  // twee regels (1 en 5 min) zonder enige aanwijzing dat 12 en 20 min misten,
  // terwijl juist DAT de reden was dat zijn FTP op korte pieken leunde
  // (23-09-2026). Nu tonen we ze als lege rij met de reden erbij. Alleen zinvol
  // als er überhaupt een curve staat, anders is de hele kaart leeg.
  const curveOntbreekt = curve.length ? curveAlle.filter(pp => !(pp[1] && pp[1] > 0)).map(pp => pp[0]) : [];
  const rennerstypeResultaat = bepaalRennerstype(ftp, piek1, wGew, piek20 || piek12 || null);
  const rennerstype = rennerstypeResultaat ? rennerstypeResultaat.type : null;
  const rennerstypeUitleg = rennerstypeResultaat ? rennerstypeResultaat.uitleg : '';
  // Vertaalt het rennerstype naar een concrete actieplan-regel: WAT betekent dit
  // type voor waar je nu op moet focussen. Dit is precies het gat tussen de
  // marketing ("sterk op 5 min, zwak op 20 min → hoe jij zou moeten trainen")
  // en het rapport zelf — het rapport berekende het type al, maar deed er tot
  // 21-09-2026 niets specifieks mee in het actieplan (Michel, 21-09-2026).
  const RENNERSTYPE_ACTIE = {
    Sprinter: 'Jouw rennerstype: Sprinter. Je explosieve vermogen is al goed ontwikkeld, dat hoef je niet apart te trainen. De grootste winst zit in je drempel: hoe hoger die staat, hoe langer je in een groep meekomt vóórdat je die sprint kunt lossen. Geef het drempelblok hieronder voorrang boven het VO2max-blok.',
    Puncheur: 'Jouw rennerstype: Puncheur. Je bent sterk in korte, harde pieken boven je drempel, precies wat je nodig hebt op korte hellingen en bij aanvallen. Kies bij het VO2max-blok hieronder de pittigere variant (optie B), zodat je die kracht ook iets langer leert vasthouden.',
    Klimmer: 'Jouw rennerstype: Klimmer. Je hebt relatief weinig sprintkracht, maar veel vermogen per kilo dat je lang volhoudt. Het drempelblok hieronder is voor jou het belangrijkste onderdeel: elke watt die je daar wint, voel je direct terug op elke helling.',
    Tijdrijder: 'Jouw rennerstype: Tijdrijder. Je curve is vlak: geen uitgesproken piek, wel een sterke motor op een constant tempo. Je top-end is daardoor relatief je zwakke plek. Neem het VO2max-blok hieronder serieus, dat is voor jou de grootste kans om te verbeteren.',
    Allrounder: 'Jouw rennerstype: Allrounder. Geen uitgesproken piek of dal in je curve, je kunt op meerdere manieren meedoen. De standaard mix van drempel en VO2max hieronder is voor jou precies goed, zonder dat één onderdeel duidelijk voorrang verdient.',
  };
  // Bij een hoge decoupling verschijnt er GEEN drempel- of VO2max-blok meer
  // hieronder (de blokken-tak verderop toont dan Duurvermogen + Tempo i.p.v.
  // de standaard blokken) — de teksten hierboven verwijzen daar toch naar, en
  // de Puncheur-tekst ("optie B" van het VO2max-blok) sprak zelfs punt 3
  // hierboven ("laat het VO2max-blok deze weken links liggen") rechtstreeks
  // tegen. Losse set teksten die wél bij Duurvermogen/Tempo passen (Joris,
  // 22-09-2026: "optie B" stond nergens in het rapport).
  const RENNERSTYPE_ACTIE_DECOUPHOOG = {
    Sprinter: 'Jouw rennerstype: Sprinter. Je explosieve vermogen is al goed ontwikkeld, dat hoef je nu niet apart te trainen. Bouw eerst je duurvermogen op zoals hierboven; zodra dat staat, is het drempelblok voor jou de grootste hefboom, hoe hoger die staat, hoe langer je in een groep meekomt vóórdat je die sprint kunt lossen.',
    Puncheur: 'Jouw rennerstype: Puncheur. Je bent sterk in korte, harde pieken boven je drempel, precies wat je nodig hebt op korte hellingen en bij aanvallen. Bouw eerst je duurvermogen op zoals hierboven; zodra dat staat, leer je die kracht met een pittiger VO2max-blok ook langer vasthouden.',
    Klimmer: 'Jouw rennerstype: Klimmer. Je hebt relatief weinig sprintkracht, maar veel vermogen per kilo dat je lang volhoudt. Bouw eerst je duurvermogen op zoals hierboven; daarna is het drempelblok voor jou het belangrijkste onderdeel, elke watt die je daar wint voel je direct terug op elke helling.',
    Tijdrijder: 'Jouw rennerstype: Tijdrijder. Je curve is vlak: geen uitgesproken piek, wel een sterke motor op een constant tempo. Bouw eerst je duurvermogen op zoals hierboven; je top-end blijft daarna je grootste kans om te verbeteren.',
    Allrounder: 'Jouw rennerstype: Allrounder. Geen uitgesproken piek of dal in je curve, je kunt op meerdere manieren meedoen. Bouw eerst je duurvermogen op zoals hierboven, daarna past de standaard mix van drempel en VO2max weer bij je.',
  };

  // ===== CURVE-BADGES — letterlijk laten zien welk punt sterk/zwak is =====
  // Rennerstype (hierboven) vertelt het verhaal via de 1-min/FTP-verhouding,
  // maar toont het niet in de curve zelf. Dit blok voegt daar een tweede,
  // ONAFHANKELIJK signaal aan toe: 5 min t.o.v. 20 min, het directe "sterk op
  // 5 min, zwak op 20 min"-paar uit de reels (of andersom bij een diesel-motor).
  // Bewust GEEN 1-min-badge hier: die zou het rennerstype-label hierboven
  // dubbelop zeggen én kan er in randgevallen tegenin lijken te gaan (bv. een
  // vlakke 1-min-curve met toch een piekje op 5 min oogt dan tegenstrijdig).
  // FTP zelf laten we ook buiten beeld, want FTP is deels AFGELEID van de
  // 20-min piek (zie FTP-formule), dus die twee tegen elkaar afzetten zou
  // circulair zijn (Michel, 21-09-2026).
  const curveBadgesRaw = bepaalCurveBadges(piek5, piek20);
  const curveBadges = {};
  for (const [k, v] of Object.entries(curveBadgesRaw)) curveBadges[k] = { tekst: v, kleur: v === 'STERK' ? GROEN : ORANJE };
  ...
  const z=(i)=>zones[i]||0;
  const laagTot=z(0)+z(1), grijsTot=z(2)+z(3), kwalTot=z(4)+z(5);
  // kwalAandeel is gewoon kwalTot: de zones tellen al op tot 100% van de totale
  // rijtijd, dus rustig+grijs+kwaliteit moet ook op 100% uitkomen. Stond hier
  // eerder als kwalTot/(laagTot+kwalTot) — dat gaf 68% een misleidende 82%,
  // want het rekende grijsTot ten onrechte weg uit de noemer (Joris, 22-09-2026).
  const kwalAandeel=kwalTot;
  ...
  //
  // TERUGGEDRAAID (23-09-2026, Tjalling): probeerde hier meta.piek12minHr/
  // piek20minHr te citeren als "bleef onder je drempel", in de veronderstelling
  // dat die cijfers altijd bij de FTP-bepalende inspanning horen. Dat klopt
  // niet — piek12minHr/piek20minHr voeden de OMSLAGPUNT-berekening en kunnen
  // van een heel andere rit komen dan de rit die de FTP-piek leverde. Bij
  // Tjalling stonden ze zelfs BOVEN zijn drempel (158/155 bpm tegen 151 bpm),
  // dus de tekst beweerde het tegenovergestelde van wat de cijfers ernaast
  // lieten zien. Zonder dat de backend exact bijhoudt welke HR bij de FTP-
  // bepalende inspanning hoorde (kiesPieken in api/strava-callback.js), is er
  // geen betrouwbare basis voor deze specifieke tekst — terug naar de
  // generieke tekst tot dat er wel is.
  //
  // TWEE SOORTEN onzekerheid, en ze wijzen NIET dezelfde kant op (Erwin,
  // 23-09-2026). Zit er wél een 12/20-min piek in de berekening maar zonder
  // HR-bewijs, dan is het getal inderdaad een bodem. Ontbreken beide lange
  // vensters (piek12/piek20 leeg), dan staat de FTP volledig op de 1- en 5-min
  // pieken, en die zijn type-afhankelijk: een diesel komt te laag uit, een
  // puncheur te hoog. Erwin kreeg 272W te lezen met "ligt waarschijnlijk hoger"
  // eronder, terwijl zijn test 240W gaf. Die zin beloofde dus precies het
  // tegenovergestelde van de werkelijkheid. Bij korte-piek-only daarom geen
  // richting claimen, en het label naast het getal is dan 'schatting', niet
  // 'ondergrens'.
  const alleenKortePieken = !!(ftp && !ftpIsHoog && !piek12 && !piek20);
  const ondergrensRegels = (ftp && !ftpIsHoog)
    ? wrapTxt(alleenKortePieken
        ? 'Dit is een schatting uit je 1- en 5-minuten pieken. In je laatste 90 dagen zit geen inspanning van 12 of 20 minuten voluit, dus dit getal kan beide kanten op: ga je kort explosief hard, dan valt het hoger uit dan je echte drempel. Rijd één keer 20 minuten voluit en je weet het zeker.'
        : 'Dit is een ondergrens. Je data mist een volle inspanning, dus je echte FTP ligt waarschijnlijk hoger. Rijd één keer voluit (20 min) en je weet het zeker.',
        reg, 8.5, R-M-32)
    : [];
  ...
  if(ftp){
    ...
    if(!ftpIsHoog){ const lbl = alleenKortePieken ? 'schatting' : 'ondergrens'; na += bold.widthOfTextAtSize('WATT',13)+10; txt(lbl,na,y-58,{font:bold,size:9,color:ORANJE}); }
  }
  ...
  // ===== HARTSLAGZONES — klein extra blok bij een betrouwbaar omslagpunt =====
  // Ook vermogen-renners hebben vaak hartslagdata, en wie daar ook naar kijkt
  // (zoals Bert, 20-09-2026) krijgt dit voortaan automatisch in het rapport
  // i.p.v. dat Michel het los per mail moet uitrekenen. Vermogen blijft leidend,
  // dit is puur een referentieblokje. Alleen bij een hoog-betrouwbaar
  // omslagpunt, anders is het een gok die de kaart niet waard is.
  const opHf = parseInt(meta.omslagpunt) || null;
  if (opHf && meta.omslagpuntBetrouwbaarheid === 'hoog') {
    const HF_ZONE_NAAM = ['Herstel','D1','D2','D3','Weerstand'];
    const hfGrenzen = (i) => {
      if (i===0) return `< ${Math.round(opHf*0.75)}`;
      if (i===HF_ZONE_NAAM.length-1) return `> ${opHf}`;
      const p=[[0,0.75],[0.75,0.85],[0.85,0.90],[0.90,1.00]]; const [lo,hi]=p[i];
      return `${Math.round(opHf*lo)}–${Math.round(opHf*hi)}`;
    };
    const hfH = 34 + HF_ZONE_NAAM.length*16 + 8;
    ensure(hfH);
    card(M,y-hfH,R-M,hfH);
    txt('HARTSLAGZONES · REFERENTIE', M+16, y-18, {font:bold,size:8,color:WIT});
    txt(`Op basis van je omslagpunt van ${opHf} bpm`, R-16, y-18, {font:reg,size:7.5,color:DIM,align:'right'});
    HF_ZONE_NAAM.forEach((nm,i)=>{ const cy=y-32-i*16; txt(nm,M+16,cy,{font:reg,size:8.5,color:MUT}); txt(`${hfGrenzen(i)} bpm`,R-16,cy,{font:bold,size:8.5,color:WIT,align:'right'}); });
    y -= hfH + 14;
  }

  // ===== PAGINA 2: AEROBE DECOUPLING + TREND + WATT/KG + POWER CURVE =====
  // Alle "verdiepende cijfers" samen op één pagina i.p.v. elk apart: los waren
  // decoupling en watt/kg allebei te leeg op hun eigen pagina (Michel, 21-09-2026).
  nieuwePagina();
  ...
  // duurrit (≥2u, steady) vond. Kon dat niet worden bepaald, dan tonen we dat
  // voortaan EXPLICIET i.p.v. de kaart stil weg te laten (Michel, 21-09-2026).
  ...
  const decouplingDatumStr = datumKort(meta.decouplingDatum);
  if (decoupling !== null && !Number.isNaN(decoupling)) {
    ...
    // decLabel staat naast "Aerobe basis:", dus dit moet de basis beschrijven,
    // niet de drift. Stond hier 'hoog' — las als een STERKE aerobe basis,
    // precies tegengesteld aan de rode kleur en de "BOUW EERST..."-pill
    // ernaast (Joris, 10.3% drift, 22-09-2026).
    else { decLabel='zwak'; decKleur=ROOD; decPil='BOUW EERST JE AEROBE BASIS'; }
    // Framing bewust op VERMOGEN i.p.v. hartslag ("je vermogen zakt weg" i.p.v.
    // "je hartslag liep op"): dit is dezelfde meting, maar dit is ook precies de
    // belofte uit de ads/reels ("je vermogen zakt 20% na 90 minuten"), dus het
    // rapport moet die belofte letterlijk terugzeggen (Michel, 21-09-2026).
    const decUitleg = decoupling <= 5
      ? 'Je vermogen bleef overeind gedurende de hele rit: je hartslag liep niet op bij gelijkblijvend vermogen, precies wat een sterke aerobe motor laat zien op de lange afstand.'
      : decoupling <= 10
      ? 'Je vermogen zakte licht weg naarmate de rit vorderde: je hartslag liep iets op bij gelijkblijvend vermogen. Geen alarm, maar met meer duurkilometers onder je basistempo houd je dit langer vast.'
      : 'Je vermogen zakte flink weg naarmate de rit vorderde: je hartslag liep duidelijk op bij gelijkblijvend vermogen. Een teken dat je aerobe basis de duur nog niet aankan, of dat vermoeidheid, hitte of te weinig vocht een rol speelde in deze rit.';
    ...
    txt(`Over je langste duurrit${decouplingMin?` · ${decouplingMin} min`:''}${decouplingDatumStr?` · ${decouplingDatumStr}`:''}`, M+16, y-76, {font:reg,size:8.5,color:MUT});
    ...
    // Niet gemeten: EXPLICIET tonen i.p.v. de kaart weg te laten, met 0% en een
    // duidelijke reden, zodat de koper niet denkt dat dit vergeten is.
    // Gepersonaliseerd met de reden (api/strava-callback.js decouplingReden):
    // bij 'te_kort' citeren we de langste rit tot nu toe als concreet doel —
    // bij 'te_onregelmatig' reed de sporter al lang genoeg, dan is "rij
    // langer" het verkeerde advies (Leon, 23-09-2026).
    const langsteRitMin = parseInt(meta.langsteRitMinuten) || null;
    const langsteRitTekst = langsteRitMin == null ? null : langsteRitMin < 60
      ? `${langsteRitMin} minuten`
      : `${Math.floor(langsteRitMin/60)} uur${langsteRitMin%60 ? ` en ${langsteRitMin%60} minuten` : ''}`;
    const nietGemetenUitleg = (meta.decouplingReden === 'te_kort' && langsteRitTekst)
      ? `Je langste rit in de laatste 90 dagen duurde ${langsteRitTekst}. We hebben minimaal 2 uur nodig om dit te meten: rijd een keer langer dan die rit en we kunnen dit bij je volgende analyse wel berekenen.`
      : meta.decouplingReden === 'te_onregelmatig'
      ? 'Je langste rit was wel lang genoeg, maar niet rustig en gelijkmatig genoeg (bijvoorbeeld een pittige rit of veel stop-and-go) om dit betrouwbaar te meten. Rijd een keer bewust rustig door en we kunnen dit bij je volgende analyse wel berekenen.'
      : 'We vonden geen kwalificerende duurrit (minimaal 2 uur, rustig en gelijkmatig) in je laatste 90 dagen om dit op te berekenen. Rijd zo’n rit en we kunnen dit bij je volgende analyse wel meten.';
    const nietGemetenLines = wrapTxt(nietGemetenUitleg, reg, 8.5, R-M-32);
    const decH = 92 + nietGemetenLines.length*11 + 6;
    ensure(decH);
    card(M,y-decH,R-M,decH,{fill:CARD2,border:BORDERO});
    txt('AEROBE DECOUPLING', M+16, y-20, {font:bold,size:9,color:ORANJE});
    txt('0%', M+16, y-58, {font:bold,size:40,color:DIM});
    txt('Pw:HR-DRIFT', M+16+bold.widthOfTextAtSize('0%',40)+8, y-58, {font:bold,size:13,color:DIM});
    const nietGemetenPil = 'KON NIET WORDEN GEDETECTEERD';
    const nietGemetenPilW = Math.max(150, bold.widthOfTextAtSize(nietGemetenPil,7.5)+20);
    page.drawRectangle({x:R-16-nietGemetenPilW,y:y-72,width:nietGemetenPilW,height:20,color:DIM});
    txt(nietGemetenPil, R-16-nietGemetenPilW/2, y-66, {font:bold,size:7.5,color:WIT,align:'center'});
    nietGemetenLines.forEach((ln,i)=> txt(ln, M+16, y-84-i*11, {font:reg,size:8.5,color:MUT}));
    y -= decH + 14;
  }
  ...
  if (meta.vorige && meta.vorige.ftp && ftp) {
    ...
    if (vorigeFtp) {
      ...
      const decRegel = (vorigeDecoupling != null && decoupling != null)
        ...
        : null;
      // Vervolgregel: nodigt uit om opnieuw te meten, ongeacht of de klant
      // vooruit- of achteruitging. Stond hier nog niet — de kaart liet zien WAT
      // er veranderde, maar zei niet wat de klant nu moest doen (Leon,
      // 23-09-2026). Staat de FTP nog op een ondergrens (ftpIsHoog false, zie de
      // FTP-kaart hierboven), dan komt er een extra zin NAAST de 90-dagen-tekst
      // (niet in plaats ervan) — een echte test op korte termijn is dan ook de
      // moeite waard, los van de 90-dagen-check (Tjalling, 23-09-2026).
      const vervolgTekst = 'Rijd de komende 90 dagen door en doe dan opnieuw een analyse om je vooruitgang te checken.'
        + ((ftp && !ftpIsHoog) ? (alleenKortePieken
            ? ` Je FTP staat nu nog op een schatting uit korte pieken: doe binnen nu en een maand een volle inspanning (bijvoorbeeld een 20-minutentest) om 'm echt vast te leggen.`
            : ` Je FTP staat nu nog op een ondergrens: doe binnen nu en een maand ook een volle inspanning (bijvoorbeeld een 20-minutentest) om 'm definitief vast te leggen.`) : '');
      const vervolgLines = wrapTxt(vervolgTekst, reg, 8.5, R-M-32);
      const laatsteVasteRegelOffset = decRegel ? 92 : 76;
      const vervolgOffset = laatsteVasteRegelOffset + 16;
      const trendH = vervolgOffset + (vervolgLines.length-1)*11 + 14;
      ensure(trendH);
      ...
      vervolgLines.forEach((ln,i)=> txt(ln, M+16, y-vervolgOffset-i*11, {font:reg,size:8.5,color:MUT}));
      y -= trendH + 14;
    }
  }
  ...
  if (ftp && wGew) {
    ...
    // piek1/piek5/piek12/piek20, curve, rennerstype en rennerstypeUitleg staan
    // hierboven al gehoist (ook nodig voor het actieplan op pagina 3).
    const rennerstypeLines = rennerstype ? wrapTxt(rennerstypeUitleg, reg, 8.5, R-M-32) : [];
    const ladderRijH = 15;
    const curveRijen = [...curve, ...curveOntbreekt.map(l => [l, null])];
    const curveUitlegLines = curveOntbreekt.length
      ? wrapTxt(`Leeg = geen volle inspanning van ${curveOntbreekt.join(' of ')} in je laatste 90 dagen. Daardoor rust je FTP-schatting op je kortere pieken en kunnen we je rennerstype nog niet bepalen. Eén keer 20 minuten voluit levert je allebei op.`, reg, 8, R-M-32)
      : [];
    const wkgH = 192 + volgLines.length*11 + (curveRijen.length ? (13 + curveRijen.length*13) : 0) + (curveUitlegLines.length ? curveUitlegLines.length*10 + 4 : 0) + (rennerstype ? (14 + rennerstypeLines.length*11) : 0) + 14;
    ensure(wkgH);
    ...
    if (curveRijen.length) {
      let cy = vy - volgLines.length*11 - 12;
      txt('JOUW POWER CURVE (W/kg)', M+16, cy, {font:bold, size:7.5, color:DIM});
      if (rennerstype) txt(`  ·  ${rennerstype.toUpperCase()}`, M+16+bold.widthOfTextAtSize('JOUW POWER CURVE (W/kg)',7.5), cy, {font:bold, size:7.5, color:ORANJE});
      cy -= 13;
      const maxWv = Math.max(...curve.map(pp => pp[1]/wGew));
      // 108 i.p.v. 70 gereserveerd rechts: ruimte voor het STERK/ZWAK-badge
      // vóór de W/kg-waarde, zodat de balken op elke rij even breed blijven,
      // ook op rijen zonder badge.
      const cbarX = M+70, cbarW = R-cbarX-108;
      curveRijen.forEach(pp => {
        // Ontbrekend venster: lege baan, geen balk, en rechts waarom het leeg is.
        if (!pp[1]) {
          txt(pp[0], M+16, cy, {font:reg, size:8.5, color:DIM});
          page.drawRectangle({x:cbarX, y:cy-2, width:cbarW, height:6, color:TRACK});
          txt('niet voluit gereden', R-16, cy, {font:bold, size:8, color:ORANJE, align:'right'});
          cy -= 13;
          return;
        }
        ...
        const badge = curveBadges[pp[0]];
        if (badge) {
          const bw = Math.max(38, bold.widthOfTextAtSize(badge.tekst,7)+12);
          const bx = R-16-70-bw;
          page.drawRectangle({x:bx, y:cy-4, width:bw, height:11, color:CARD, borderColor:badge.kleur, borderWidth:0.8});
          txt(badge.tekst, bx+bw/2, cy-1, {font:bold, size:6.5, color:badge.kleur, align:'center'});
        }
        txt(`${wv.toFixed(1)} W/kg`, R-16, cy, {font:bold, size:8.5, color:MUT, align:'right'});
        cy -= 13;
      });
      if (curveUitlegLines.length) { cy += 2; curveUitlegLines.forEach(ln => { txt(ln, M+16, cy, {font:reg, size:8, color:DIM}); cy -= 10; }); cy -= 2; }
      if (rennerstype) { cy -= 1; rennerstypeLines.forEach(ln => { txt(ln, M+16, cy, {font:reg, size:8.5, color:MUT}); cy -= 11; }); }
    }
    y -= wkgH + 14;
  }

  // ===== PAGINA 3: AANDACHTSPUNT + ACTIEPLAN + INTERVALBLOKKEN + WEEK =====
  nieuwePagina();
  ...
  if (decoupHoog) {
    acties = [
      'Duurvermogen is nu je belangrijkste werk, niet extra intensiteit. Je vermogen zakte flink weg tijdens je langste rit, een teken dat je aerobe basis de duur nog niet aankan.',
      rustigRegel,
  ...
  // Vierde, type-specifieke regel: WAT je curve-vorm betekent voor waar je nu op
  // moet focussen (zie RENNERSTYPE_ACTIE/RENNERSTYPE_ACTIE_DECOUPHOOG hierboven).
  // Pas hier gekozen (i.p.v. bij het hoisten van rennerstype) omdat decoupHoog
  // pas op dit punt bekend is.
  const rennerstypeActieTekst = rennerstype
    ? (decoupHoog ? RENNERSTYPE_ACTIE_DECOUPHOOG[rennerstype] : RENNERSTYPE_ACTIE[rennerstype])
    : null;
  if (rennerstypeActieTekst) acties.push(rennerstypeActieTekst);
  ...
  // 'Van inzicht naar uitvoering'-CTA is bewust UIT het PDF gehaald (Michel,
  // 21-09-2026): de upsell naar het trainingsschema staat al in de klantmail
  // zelf (zie klantHtml hieronder), dat hoeft niet dubbel in de bijlage.
...
function klantHtml(naam, token, deadline, advies, isHartslag, m, vorige) {
  ...
  // Diagnose (decoupling-signaal, rennerstype, teaser, upsell-reden) komt uit
  // bepaalKoperDiagnose hierboven — dezelfde functie die interneHtml gebruikt,
  // zodat Michel in zijn eigen verkoopmail precies ziet wat de klant hier leest.
  const { teaserRegel, upsellReden } = bepaalKoperDiagnose(mm, isHartslag);
  ...
  <div style="font-family:Arial,Helvetica,sans-serif;color:#1a1a1a;line-height:1.65;max-width:560px;">
    ...
    <div style="margin:22px 0;padding:22px;background:#0d0d0d;border-radius:12px;">
      ...
      <p style="font-size:15px;color:#ffffff;margin:0 0 8px;">Ons advies: het <strong style="color:#ff6b1a;">${a.niveau} ${a.weken}-wekenplan</strong>.${prijsRegel}</p>
      <p style="font-size:14px;color:#c8c8c8;margin:0 0 16px;">${upsellReden}</p>
...
function interneHtml(m, bedrag, id, pdfErbij, vorige) {
  ...
  // Zelfde diagnose als de klantmail (teaser + upsell-reden) en, bij vermogen,
  // het rennerstype + de curve-badges uit de PDF. Michel wilde dit advies ook
  // zelf zien i.p.v. alleen de ruwe cijfers (Michel, 21-09-2026).
  const diagnose = bepaalKoperDiagnose(m, isHartslagNu);
  const curveBadgesIntern = !isHartslagNu ? bepaalCurveBadges(parseInt(m.piek5min)||null, parseInt(m.piek20min)||null) : {};
  const curveBadgeTekst = Object.entries(curveBadgesIntern).map(([k,v]) => `${k} ${v.toLowerCase()}`).join(', ');
  const adviesBalk = `<p style="margin:16px 0 0;padding:10px 14px;border-radius:6px;background:#eef2ff;color:#2b3a8f;font-size:14px;line-height:1.6;">💡 <strong>Wat de klant net te lezen kreeg</strong> — ${diagnose.teaserRegel} <em>Upsell-reden:</em> ${diagnose.upsellReden}</p>`;
  ...
  <div style="font-family:Arial,sans-serif;color:#111;line-height:1.6;">
    ...
    <table style="border-collapse:collapse;font-size:15px;">
      ...
      ${r('Bron', escHtml(m.bron||'onbekend'))}
      ${r('Meetmethode', escHtml(m.meetmethode==='hartslag' ? 'hartslag (omslagpunt)' : 'vermogen (FTP)'))}
      ${r('Rennerstype', diagnose.rennerstype ? escHtml(diagnose.rennerstype.type) : '—')}
      ${r('Curve-signaal (5 vs 20 min)', curveBadgeTekst ? escHtml(curveBadgeTekst) : '—')}
...
// ===== ONTDUBBELING VAN DE NURTURE-STAP BINNEN ÉÉN BESTELLING =====
// naarMailchimp schrijft de merge fields (FTP/DECOUPL/...) VOORDAT de PDF of
// de klantmail geslaagd zijn. Faalt een van die twee, dan geeft de webhook een
// 503 terug zodat Mollie dezelfde betaling later opnieuw aanbiedt (zie
// betaling-webhook.js) — en die retry roept leverRapport nog een keer aan met
// hetzelfde bestel-id. haalVorigeAnalyse leest dan de merge fields terug die
// de EIGEN, mislukte eerste poging net wegschreef, en denkt: "vorige analyse
// gevonden" — een bestelling die met zichzelf wordt vergeleken en zich als
// herhaalkoper voordoet, terwijl er maar één (nooit afgeleverde) analyse was
// (Joris: "vorige" FTP 161W = "nu" FTP 161W, maar er was nooit een eerste
// mail verstuurd, 22-09-2026). Per bestel-id (Mollie-betaal-id of
// 'gratis-<kortingId>') onthouden we daarom of de nurture-stap al eens is
// geprobeerd; bij een volgende poging voor hetzelfde id negeren we 'vorige'.
// Redis onbereikbaar → fail open (net als de rest van deze module): dan is het
// risico een enkele onterechte "Herhaalkoper"-melding, geen blokkade.
const NURTURE_REDIS_URL   = process.env.UPSTASH_REDIS_REST_URL   || process.env.KV_REST_API_URL;
const NURTURE_REDIS_TOKEN = process.env.UPSTASH_REDIS_REST_TOKEN || process.env.KV_REST_API_TOKEN;
async function nurtureRedis(cmd) {
  if (!NURTURE_REDIS_URL || !NURTURE_REDIS_TOKEN) return { ok: false };
  try {
    const r = await fetch(NURTURE_REDIS_URL, {
      method: 'POST',
      headers: { Authorization: `Bearer ${NURTURE_REDIS_TOKEN}`, 'Content-Type': 'application/json' },
      body: JSON.stringify(cmd),
      signal: AbortSignal.timeout(5000)
    });
    if (!r.ok) { console.error('Redis fout (nurture-ontdubbeling):', r.status); return { ok: false }; }
    const j = await r.json();
    return { ok: true, result: j.result };
  } catch (e) { console.error('Redis exception (nurture-ontdubbeling):', e); return { ok: false }; }
}
async function isHerhaaldeNurturePoging(id) {
  if (!id) return false;
  const r = await nurtureRedis(['SET', `pp:nurture-poging:${id}`, '1', 'NX', 'EX', '604800']);
  if (!r.ok) return false; // storing: fail open
  return r.result !== 'OK'; // 'OK' = net gezet (eerste poging), anders bestond de sleutel al
}

// bepaalPijn/bepaalDecouplingSignaal staan nu in ./pijn-signalen.js, gedeeld
// met api/lead.js (verlaten-journey gebruikt dezelfde diagnose).
...
// Leest de KOOP*-velden i.p.v. FTP/DECOUPL/OMSLAG/DECOUPLHR: die laatste worden
// ook door api/lead.js geschreven zodra iemand in de funnel afhaakt (voor de
// verlaten-journey-personalisatie), dus een lead die na een echte aankoop nog
// eens afhaakt overschrijft daarmee stilletjes wat hier als "vorige analyse"
// zou tellen. KOOP* wordt UITSLUITEND hieronder in naarMailchimp gezet, ná een
// daadwerkelijk afgeleverde (betaalde) analyse, dus die kunnen niet besmet
// raken door een niet-afgeronde funnel-poging (Barry: echte vorige aankoop
// 236W op 6 juli, maar een latere afgehaakte sessie overschreef FTP naar 247W
// vóórdat hij vandaag opnieuw kocht — de vergelijking toonde daardoor 247→247
// i.p.v. 236→247, 24-09-2026).
async function haalVorigeAnalyse(base, auth, hash) {
  try {
    ...
    if (!mf.KOOPFTP && !mf.KOOPDEC && !mf.KOOPOMS && !mf.KOOPDCHR) return null;
    return {
      ftp: mf.KOOPFTP || '', score: mf.SCORE || '', decoupling: mf.KOOPDEC || '', datum: mf.RAPDAT || '',
      omslagpunt: mf.KOOPOMS || '', decouplingHr: mf.KOOPDCHR || ''
    };
  } catch (e) { console.error('Vorige analyse ophalen faalde (blokkeert niet):', e); return null; }
}

async function naarMailchimp(m, token, deadlineNL, advies, isRetry) {
  ...
  const { bijpijn, decouplTxt } = bepaalDecouplingSignaal(m);
  // Rennerstype voor de koper-journey (RENTYPE-merge-field), zelfde bron als
  // het PDF-actieplan en de klantmail-teaser (Michel, 21-09-2026).
  const wGewVoorMc = (() => { const w = parseFloat(m.weight); return (w >= 35 && w <= 200) ? w : null; })();
  const rennerstypeVoorMc = bepaalRennerstype(parseInt(m.ftp)||null, parseInt(m.piek1min)||null, wGewVoorMc, (parseInt(m.piek20min)||parseInt(m.piek12min)||null));
  ...
  // Retry van dezelfde bestelling → 'vorige' zou onze eigen vorige poging zijn,
  // niet een echte eerdere analyse. Zie de uitleg bij isHerhaaldeNurturePoging.
  const vorige = isRetry ? null : await haalVorigeAnalyse(base, auth, hash);
  try {
    ...
    await fetch(`${base}/members/${hash}`, {
      ...
      body: JSON.stringify({
        ...
        merge_fields: {
          ...
          // KOOP*: alleen hier gezet, ná een daadwerkelijk afgeleverde (betaalde)
          // analyse — in tegenstelling tot FTP/DECOUPL/OMSLAG/DECOUPLHR hierboven,
          // die ook door api/lead.js worden overschreven bij een afgehaakte
          // funnel-sessie. haalVorigeAnalyse leest daarom KOOP*, niet deze
          // (Barry, 24-09-2026).
          KOOPFTP: m.ftp || '', KOOPDEC: m.decoupling != null && m.decoupling !== '' ? String(m.decoupling) : '',
          KOOPOMS: m.omslagpunt || '', KOOPDCHR: m.decouplingHr != null && m.decouplingHr !== '' ? String(m.decouplingHr) : '',
          PIJN: pijn, PIJNPCT: String(pct),
          BIJPIJN: bijpijn, DECOUPLTXT: decouplTxt,
          RENTYPE: rennerstypeVoorMc ? rennerstypeVoorMc.type : '',
...
export async function leverRapport(m, { bedrag, id } = {}) {
  ...
  let vorige = null;
  try {
    const isRetry = await isHerhaaldeNurturePoging(id);
    const resultaat = await naarMailchimp(m, ppToken, ppDeadline, advies, isRetry);
